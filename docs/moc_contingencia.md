...
- 2 Contingência da NF-e (modelo 55)
   - 2.1. Modalidades de Emissão de NF-e
      ...
      - Documento Fiscal Eletrônico – FS-DA 2.1.2. Contingência em Formulário de Segurança para impressão de Documento Auxiliar de
...
**7 .00** Novembro/2020 Criação deste manual como documento anexo do MOC. Corresponde ao capítulo^8
...
```
```
7.00 •^ Separação do capítulo específico. 8 -^ Contingência do MOC 6.0,^ para este manual
```
...
será impresso em papel comum após o recebimento da autorização de uso da NF-e;
...
possível utilizando-se formulários de segurança para impressor autônomo, nos termos da legislação vigente
...
d) SVC – Sefaz Virtual de Contingência – é alternativa de emissão de NF-e em contingência com trans mis s ão
...
(^1) Se a empresa estiver utilizando seu estoque de FS-IA nos termos do Convênio ICMS 58/95, deverá utilizar o
campo tpEmis com valor “ 2 ”^1
...
O serviço de recepção e autorização de NF-e pela SVC (Web Service: NFeAutorizacao) somente
...
Nota: No momento que a empresa detecta a indisponibilidade do ambiente de autorização normal,
...
o ambiente de produção e [http://hom.nfe.fazenda.gov.br](http://hom.nfe.fazenda.gov.br) para o ambiente de homologação).
...
O FS–IA possui filigrana caracterizada com o brasão de Armas da República intercalada com a
...
Eletrônicos. Estas filigranas somente se tornam visíveis contra a luz, conformes exemplos e modelos
...
(^2) Observar que a manutenção do número e série somente se aplica para os casos de rejeição da NF-e que foi
emitida em contingência, e nunca para os casos em que a NF-e foi normalmente emitida mas o contribuinte
não obteve êxito na consulta sobre o resultado da autorização de uso de uma NF-e emitida com _tpEmis = “1”_
